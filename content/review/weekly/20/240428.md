...
4. [ ] 间隔
   ...
   6. [ ] 出手办
      2. [ ] 5~6月，闲鱼先把高价的出了。(不是很想卖，都是自己用心买的)
      3. [ ] 临近毕业的几天，同学里：先问有没有玩手办的，可以低价出。然后卖不出价的还不如送掉(spq,yfy,)。最后是暂存一些到spq、yfy或者sjf那里，等到华为了再寄出去。
      1. [ ] 家里寄回两个~三个。和家里沟通的态度，同学要处理手办，低价买了几个。渐进一些好接受一些。
      4. [ ] 最后最好保持在三~四个左右有亮点的精品(每两年可以换一个，闲鱼高价)。景品留一两个(可爱、温暖型)即可。
   7. [ ] WARP网络配置
